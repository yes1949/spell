...
<body>
  ...
  <div class="main-content" id="mainContainer">
    ...
    <div id="page-drill" class="page active">
      ...
      <div class="version-display">Version: 2.2</div>
...
<script>
  ...
  // --- 数据持久化 ---
  function loadData() {
    const data = localStorage.getItem("myCardsData_Final_V2.1");
    if (data) {
  ...
  function saveData() {
    ...
    localStorage.setItem("myCardsData_Final_V2.1", JSON.stringify(data));
    updateReviewBadge(); 
  }
  ...
  function updateProgressBar() {
    ...
    if (!activeWord) { disp.textContent = "0 / 0"; return; }
  ...
  document.getElementById("wordInput").addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      if (val.toLowerCase() === activeWord.word.toLowerCase()) {
        ...
        // 延迟恢复释义 (不阻塞输入)
        setTimeout(() => {
            // 简单校验：防止跳太快把新词的释义给覆盖了
            // 但因为是单线程，且用户大概率还在看，直接恢复通常体验最好
            if (defDisplay.textContent.includes(correctWord)) {
